...
const rtA = new THREE.WebGLRenderTarget(width, height, {
	// a WebGL render target is an offscreen texture that can be used to render to, this is useful because we can use these render targets to store in-between textures
...
const rayMaterial = new THREE.ShaderMaterial({
	...
	vertexShader: paintMaterial.vertexShader,
	// this is the raymarching shader; the main difference from main.js is that we raymarch in multiple passes and merge them together. Each pass has a different resolution and amount of rays
	fragmentShader: ` // this needs more explaining, right now it doesn't cover the entire idea of radiance cascades
    precision highp float;
